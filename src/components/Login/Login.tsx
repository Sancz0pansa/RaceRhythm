  ...
  return (
    ...
    <div className={`login-container ${loginVisible && 'visible'}`}>
        ...
        <form className='login-container__form' onSubmit={(e) => submit(e)}>
            ...
            <input className='login-container__form--text' disabled={isLoading} name='login' type="text" value={login} onChange={(e) => setLogin(e.target.value)} required/>
            <label htmlFor="password">{lang === 'PL' ? 'Hasło:' : 'Password:'}
            </label>
            <input className='login-container__form--text' disabled={isLoading} name='password' type="password" value={password} onChange={(e) => setPassword(e.target.value)} required/>
            ...
            <input className='login-container__form--text' disabled={isLoading} name='email' type="email" value={email} onChange={(e) => setEmail(e.target.value)} required/></>}
            <input className='login-container__form--submit' disabled={isLoading} type="submit" value={lang === 'PL' ? registerMode === false ? 'Zaloguj się' : 'Zarejestruj się' : registerMode === true ? 'Register' : 'Login'} />
